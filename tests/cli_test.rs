...
fn test_cli_json_output() {
    let fixture_dir = concat!(
        ...
        "/tests/fixtures/processor/with_read_terragrunt_config"
    );

    let output = cargo_bin()
        .args(["--format", "json", fixture_dir])
        .output()
        .expect("Failed to execute");
...
fn test_cli_atlantis_output() {
    ...
    let output = cargo_bin()
        .args(["--format", "atlantis", fixture_dir])
        .output()
        .expect("Failed to execute");
...
fn test_cli_digger_output() {
    ...
    let output = cargo_bin()
        .args(["--format", "digger", fixture_dir])
        .output()
        .expect("Failed to execute");
...
fn test_cli_verbose_output() {
    ...
    let output = cargo_bin()
        .args(["--verbose", "--format", "json", fixture_dir])
        .output()
        .expect("Failed to execute");
...
fn test_cli_empty_directory() {
    let fixture_dir = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/tests/fixtures/parser/no_projects"
    );
